...
const doReview = async () => {
  try {
    const resp = await doPictureReview({
      id: modalInfo.id,
...
const fetchPictureList = async () => {
  const resp = await listPictureByPage({
    ...searchParams,
  })
  const res = resp.data
  if (res.code === 20000 && res.data) {
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.dataIndex === 'picInfo'">
            <div class="picInfo">
              ...
              <div>大小:{{ formatPictureSize(record.picSize) }}</div>
            </div>
          </template>
...
#pictureManage :deep(.ant-form-item-label) {
  font-size: 16px;
  font-weight: 400;
}
